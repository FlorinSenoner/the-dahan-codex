...
function areGamesEqual(row: ParsedGameRow, existing: ExistingGame): boolean {
  ...
  for (let i = 0; i < rowSpirits.length; i++) {
    ...
    if ((rowSpirit.variant || '') !== (existingSpirit.variant || '')) return false
    if ((rowSpirit.player || '') !== (existingSpirit.player || '')) return false
  }
  ...
  if (rowHasAdversary && existing.adversary) {
    ...
    if (parseInt(row.adversary_level, 10) !== existing.adversary.level) return false
  }
  ...
  if (rowHasSecondary && existing.secondaryAdversary) {
    if (row.secondary_adversary !== existing.secondaryAdversary.name) return false
    if (parseInt(row.secondary_adversary_level, 10) !== existing.secondaryAdversary.level)
      return false
  }
  ...
  if (rowHasScenario && existing.scenario) {
    ...
    const rowDiff = row.scenario_difficulty ? parseInt(row.scenario_difficulty, 10) : undefined
  ...
  const rowInvaderStage = row.invader_stage ? parseInt(row.invader_stage, 10) : undefined
  if (rowInvaderStage !== existing.invaderStage) return false

  const rowBlightCount = row.blight_count ? parseInt(row.blight_count, 10) : undefined
  if (rowBlightCount !== existing.blightCount) return false

  const rowDahanCount = row.dahan_count ? parseInt(row.dahan_count, 10) : undefined
  if (rowDahanCount !== existing.dahanCount) return false

  const rowCardsRemaining = row.cards_remaining ? parseInt(row.cards_remaining, 10) : undefined
...
export function validateParsedGame(
  ...
  if (!row.date) {
    ...
    errors.push('Invalid date format (expected YYYY-MM-DD, DD.MM.YYYY, or MM/DD/YYYY)')
  }
  ...
  const hasSpirit =
    row.spirit1 || row.spirit2 || row.spirit3 || row.spirit4 || row.spirit5 || row.spirit6
  if (!hasSpirit) {
  ...
  const existingGame = row.id ? existingGames.find((g) => g._id === row.id) : undefined
