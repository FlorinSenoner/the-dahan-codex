import type { GameFormData, SpiritEntry } from "@/components/games/game-form";
...
/**
 * Validate that game form data has at least one valid spirit
 * Returns the valid spirits or null if invalid
 */
export function getValidSpirits(spirits: SpiritEntry[]): SpiritEntry[] | null {
  const validSpirits = spirits.filter((s) => s.spiritId !== null || s.name);
  return validSpirits.length > 0 ? validSpirits : null;
}
