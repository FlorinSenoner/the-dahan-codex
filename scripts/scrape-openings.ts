          ...
          if (redirectUrl) {
            fetchPage(
              redirectUrl.startsWith("http")
                ? redirectUrl
                : `${LATENTOCTOPUS_BASE}${redirectUrl}`,
            )
              .then(resolve)
              .catch(reject);
            return;
...
function parseLatentoctopusGuide(
  html: string,
  config: GuideConfig,
): ScrapedOpening | null {
  ...
  const uniqueTurns = turns.filter(
    (t, i, arr) => i === 0 || t.turn !== arr[i - 1].turn,
  );
...
async function scrapeLatentoctopusGuide(
  config: GuideConfig,
): Promise<ScrapedOpening | null> {
  const url = `${LATENTOCTOPUS_BASE}${config.path}`;
  console.log(
    `Fetching ${config.spiritName} Opening ${config.openingNumber}...`,
  );
  console.log(`  URL: ${url}`);
...
async function main() {
  ...
  if (args.includes("--dry-run")) {
    ...
    for (const config of LATENTOCTOPUS_GUIDES) {
      console.log(
        `  - ${config.spiritName} Opening ${config.openingNumber}: ${config.path}`,
      );
    }
