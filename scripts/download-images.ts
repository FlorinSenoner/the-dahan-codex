  ...
  $('img').each((_, el) => {
    ...
    if (isExactMatch || hasSrcMatch) {
      ...
      if (!imageUrl && src.includes('/thumb/')) {
        ...
        const thumbMatch = src.match(/\/images\/thumb(\/[a-f0-9]\/[a-f0-9]{2}\/[^/]+\.png)/i)
        if (thumbMatch) {
...
function findAspectImageUrl(
  ...
  const normalizedPattern = imagePattern.replace('.png', '').replace(/ /g, '_').toLowerCase()
  ...
  $('img').each((_, el) => {
    ...
    const matchesPattern =
      normalizedSrc.includes(normalizedPattern) || normalizedAlt.includes(normalizedPattern)
...
async function main() {
  ...
  if (!fs.existsSync(spiritsPath)) {
    console.error(`Error: ${spiritsPath} not found. Run scrape-spirits.ts first.`)
    process.exit(1)
  }
