...
**IMPORTANT:** Ensure ALL uniquePowers entries for both spirits have range and target fields populated. Verify no power card is missing these fields.
  </action>
  <verify>
1. Run `npx convex run seed:reseedSpirits` to verify data seeds correctly.
2. Check Convex dashboard that spirits have new data structure.
3. Verify ALL uniquePowers have range and target fields: Query the database and confirm no power card has undefined range or target.
  </verify>
  <done>
River and Lightning have correct growth options (G1/G2/G3), presence tracks in new array format, and ALL uniquePowers with range/target fields populated.
  </done>
</task>
...
5. All uniquePowers have range and target fields populated (not undefined)
</verification>
...
- ALL uniquePowers include range and target fields (mandatory, not optional in seed data)
