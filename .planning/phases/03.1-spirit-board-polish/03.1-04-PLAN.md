...
must_haves:
  truths:
    - "User sees 3 growth options for River spirit (not 2)"
    - "User sees visual icons representing each growth action"
    - "User can identify which option is G1, G2, or G3 on hover"
    - "Growth panel gracefully handles old data format without crashing"
  artifacts:
...
2. **IMPORTANT: Add backward compatibility check FIRST before rendering:**
```typescript
function isNewGrowthFormat(growth: unknown): growth is GrowthPanelProps["growth"] {
  return (
    growth !== null &&
    typeof growth === "object" &&
    "options" in growth &&
    Array.isArray((growth as any).options)
  );
}

export function GrowthPanel({ growth }: { growth: unknown }) {
    ...
    return (
      <section className="space-y-3">
        ...
        <Text variant="small" className="text-muted-foreground">
          Growth data is in legacy format. Please reseed spirits to see updated growth options.
        </Text>
      </section>
    );
  }

  // Proceed with new format rendering
  return <GrowthPanelContent growth={growth} />;
}
```

3. Create GrowthOption component for each option card:
```typescript
...
4. Create GrowthActionBadge component to render each action with icon:
```typescript
...
5. Format function for short action labels:
```typescript
...
6. Main component layout:
```typescript
function GrowthPanelContent({ growth }: GrowthPanelProps) {
  return (
    <section className="space-y-3">
...
Also verify: If database has old format, component shows graceful fallback message instead of crashing.
  </verify>
  <done>
Growth panel displays G1/G2/G3 options with SVG icons for each action type. Labels visible on hover. Handles old data format gracefully.
  </done>
</task>
...
6. Old data format shows helpful message instead of crash
</verification>
...
- Backward compatible with graceful fallback for old data format
</success_criteria>
