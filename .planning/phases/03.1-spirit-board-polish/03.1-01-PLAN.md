...
Reference spirit-colors.ts for the existing `--element-*` CSS color tokens.
  ...
  <verify>
Run `pnpm typecheck` - no errors in icons directory.
Verify each icon file exists and exports properly.
  </verify>
  <done>
8 element icon components exist with consistent API, barrel export provides ElementIcon mapping for dynamic rendering.
  </done>
</task>
