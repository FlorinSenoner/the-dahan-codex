...
must_haves:
  truths:
    - "Schema explicitly defines valid element values including Any and Star"
  ...
  artifacts:
    - path: "convex/schema.ts"
      provides: "Element union type with all valid element values"
      contains: "elementValidator"
  ...
  key_links:
    - from: "convex/schema.ts"
      to: "presenceTracks.nodes"
      via: "elementValidator for elements field"
      pattern: "elements.*elementValidator"
...
Purpose: Some spirits have "Any" (wildcard) or "Star" (special) elements on their presence tracks. Currently the schema only supports arbitrary strings and PresenceNode shows elements as text in tooltips only.

Output: Schema with explicit element type, new element icons, and visual rendering in presence slots
</objective>
...
<task type="auto">
  <name>Task 0: Update schema with explicit element union type</name>
  <files>convex/schema.ts</files>
  <action>
Add an explicit element validator union type that includes all 8 basic elements plus "Any" and "Star".
...
Then update the presenceTracks.nodes.elements field (around line 158) from:
```typescript
...
This provides:
1. TypeScript knows which element values are valid
2. Schema is self-documenting
3. Invalid element strings will be caught at validation time
  </action>
  <verify>
`pnpm typecheck` passes
grep for "elementValidator" in schema.ts
grep for "Any" and "Star" in elementValidator definition
  </verify>
  <done>
Schema explicitly defines all valid element values including Any and Star.
  </done>
</task>
...
<verification>
1. Schema has elementValidator union with all 10 element types
2. presenceTracks.nodes.elements uses elementValidator
3. Any and Star icon files exist
4. ElementIcon record has Any and Star
5. PresenceNode renders element icons visually
6. Tooltips explain Any and Star mechanics
7. `pnpm typecheck` passes
</verification>

<success_criteria>
- Schema: elementValidator with Sun, Moon, Fire, Air, Water, Earth, Plant, Animal, Any, Star
