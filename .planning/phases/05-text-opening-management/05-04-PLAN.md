...
```
  ...
  <verify>pnpm typecheck passes</verify>
  <done>EditableOpening component created with turn management</done>
</task>

<task type="auto">
  <name>Task 3: Update OpeningSection for edit mode</name>
...
// If not editing: show existing display (TurnAccordion)
```

Note: Keep the existing display logic for non-edit mode. The edit UI should layer on top.
  </action>
  <verify>pnpm typecheck passes, pnpm dev shows edit mode toggle working</verify>
  <done>OpeningSection shows editable UI for admins in edit mode</done>
</task>
...
3. EditableOpening renders with all fields editable
