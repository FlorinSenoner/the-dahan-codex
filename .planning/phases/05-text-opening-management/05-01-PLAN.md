...
1. Update convex/schema.ts openings table:
   - Remove difficulty field entirely (we control seed data)
   - Add createdAt: v.number() - required, mutations always write Date.now()
   - Add updatedAt: v.number() - required, mutations always write Date.now()
...
3. Update seed data (convex/seed.ts or equivalent) to include createdAt and updatedAt fields for existing openings.
  </action>
  <verify>pnpm typecheck passes, npx convex dev shows no schema errors</verify>
  <done>Schema updated with required timestamps, mutations created and protected by requireAdmin</done>
</task>
...
- Schema has required createdAt/updatedAt fields on openings table
