...
4. Add blocker state for AlertDialog navigation warning. Replace any existing useBlocker call.
...
Then add the useBlocker with modern TanStack Router API (using shouldBlockFn + withResolver):
```typescript
...
Note: The modern API uses `shouldBlockFn` (a function that returns boolean) instead of the deprecated `condition` property. Setting `withResolver: true` returns a blocker object with `status`, `proceed()`, and `reset()` methods.
