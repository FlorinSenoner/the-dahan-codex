...
must_haves:
  truths:
    ...
    - "Seed openings are re-created if missing after reseed"
    - "Multiple reseeds do not create duplicate openings"
    - "Openings for spirits that no longer exist are gracefully skipped"
  artifacts:
    - path: "convex/seed.ts"
  ...
  key_links:
    ...
    - from: "convex/seed.ts:restoreOpenings"
      to: "convex/seed.ts:insertSeedData"
      via: "uses spiritIdsBySlug map"
      pattern: "spiritIdsBySlug\\.get"
---
...
2. After seeding spirits:
   ...
   - Skip if spirit no longer exists (edge case - log warning)
...
4. Handle idempotency:
   - Check if opening with same slug already exists before inserting
   - Skip duplicates to prevent multiple reseeds creating duplicates
...
- Gracefully handles spirits that no longer exist
  </action>
...
2. Add restore function with duplicate and missing spirit handling:
```typescript
async function restoreOpenings(
  ...
  for (const backup of backups) {
    ...
    // Handle spirits that no longer exist
    if (!spiritId) {
      console.warn(`Opening orphaned: spirit "${backup.spiritSlug}" no longer exists`);
      orphaned++;
      continue;
    }

    // Check if opening with same slug already exists for this spirit (idempotency)
...
export const reseedSpirits = mutation({
  handler: async (ctx) => {
    ...
    // Restore openings
    const { restored, skipped, orphaned } = await restoreOpenings(ctx, openingBackups, spiritIdsBySlug);

    return {
      status: "reseeded",
      message: `Created ${stats.expansions} expansions, ${stats.spirits} spirits, ${stats.aspects} aspects. ` +
...
4. Ensure insertSeedData returns spiritIdsBySlug map (from Plan 08-04)
  </action>
  <verify>
TypeScript compiles: npx convex dev --once
Run local test with mock data to verify backup/restore
  </verify>
  <done>reseedSpirits backs up and restores openings during reseed with idempotency and orphan handling</done>
</task>
...
5. Run again to test idempotency:
   ...
   - Return message shows "X skipped (duplicates)"

6. Test edge case - orphaned openings:
   - (Optional) Temporarily remove a spirit from seed data
   - Verify orphaned openings are logged but don't break reseed
   - Restore spirit to seed data

7. Update return message to show accurate counts
  </action>
  <verify>
Run: npx convex run seed:reseedSpirits
Check: Openings preserved after reseed (verify in dashboard)
Run again: npx convex run seed:reseedSpirits (verify no duplicates, shows "skipped" count)
  </verify>
  <done>Opening preservation verified working correctly with idempotency and edge case handling</done>
</task>
...
4. No duplicate openings created on repeated reseed (idempotent)
5. Orphaned openings (missing spirit) are gracefully skipped with warning
6. Return message shows accurate restoration counts including skipped and orphaned
</verification>
...
- Mutation is idempotent (multiple runs produce same result)
- Graceful handling of edge cases (orphaned openings, duplicates)
</success_criteria>
